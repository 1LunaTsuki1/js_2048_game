 ...
 */ class Game {
   ...
   */ constructor(initialState = [
    ...
    _processRow(row) {
        ...
        const filterArray = [];
        for(let i = 0; i < row.length; i++)if (row[i] !== 0) filterArray.push(row[i]);
        const mergeArray = [];
        for(let i = 0; i < filterArray.length; i++){
            ...
            if (i < filterArray.length - 1 && filterArray[i] === filterArray[i + 1]) {
                ...
                i = i + 1;
                this.score += doubled;
            } else mergeArray.push(filterArray[i]);
        }
        while(mergeArray.length < 4)mergeArray.push(0);
        return mergeArray;
    }
    _transpose(matrix) {
    ...
    moveLeft() {
        ...
        if (initialBoardString !== endBoardString) this._spawn();
    }
    moveRight() {
        const initialBoardString = JSON.stringify(this.board);
